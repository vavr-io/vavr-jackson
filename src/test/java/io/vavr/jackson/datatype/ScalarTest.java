...
class ScalarTest extends BaseTest {
    ...
    void shouldDeserializeIntegerList() {
        List<Integer> l1 = mapper().readValue("[1]", new TypeReference<>() {
        });
    ...
    void shouldDeserializeBooleanList() {
        List<Boolean> l2 = mapper().readValue("[true]", new TypeReference<>() {
        });
        assertThat(List.of(true)).isEqualTo(l2);

        List<Boolean> l3 = mapper().readValue("[false]", new TypeReference<>() {
        });
    ...
    void shouldDeserializeFloatList() {
        List<Float> l4 = mapper().readValue("[2.4]", new TypeReference<>() {
        });
    ...
    void shouldDeserializeDoubleAndBigDecimalList() {
        List<Double> l5 = mapper().readValue("[2.4]", new TypeReference<>() {
        });
        ...
        List<BigDecimal> l5d = mapper().rebuild().enable(DeserializationFeature.USE_BIG_DECIMAL_FOR_FLOATS).build()
            .readValue("[2.4]", new TypeReference<>() {
            });
    ...
    void shouldDeserializeStringList() {
        List<String> l6 = mapper().readValue("[\"1\"]", new TypeReference<>() {
        });
    ...
    void shouldDeserializeLongList() {
        List<Long> l7 = mapper().readValue("[24]", new TypeReference<>() {
        });
        assertThat(List.of(24L)).isEqualTo(l7);

        List<Long> l8 = mapper().readValue("[" + Long.MAX_VALUE + "]", new TypeReference<>() {
        });
    ...
    void shouldDeserializeBigIntegerList() {
        List<BigInteger> l9 = mapper().readValue("[1234567890123456789012]", new TypeReference<>() {
        });
